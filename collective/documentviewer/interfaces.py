...
class IGlobalDocumentViewerSettings(Interface):
    large_size = schema.Int(title=_("Large Image Size"), default=1000)
    normal_size = schema.Int(title=_("Normal Image Size"), default=700)
    thumb_size = schema.Int(title=_("Thumb Image Size"), default=180)
    storage_type = schema.Choice(
        title=_("Storage Type"),
        description=_(
            "Blob storage using the normal ZODB blob mechanism. "
    ...
    storage_location = schema.TextLine(
        ...
        description=_("Only for file storage not with zodb. " "Plone client must have write access to directory."),
        default=u"/opt/dvpdffiles",
    )
    storage_obfuscate = schema.Bool(
        title=_("Obfuscate private file paths"),
        description=_(
            "*experimental* If you're serving files straight from "
    ...
    pdf_image_format = schema.Choice(
        title=_("Image Format"), default=u"gif", vocabulary=SimpleVocabulary.fromValues(["gif", "png", "jpg"])
    )
    enable_indexation = schema.Bool(
        title=_("Make searchable"),
        description=_(
            "If this is enabled, the text will be extracted from "
    ...
    ocr = schema.Bool(
        title=_("OCR"), description=_("Extract the text from the PDF using OCR technology."), default=False
    )
    detect_text = schema.Bool(
        title=_("Detect text"),
        description=_(
            "Detect if pdf has text before performing OCR on it. "
    ...
    auto_select_layout = schema.Bool(title=_("Auto select layout"), description=_("For pdf files."), default=True)
    auto_layout_file_types = schema.List(
        title=_("Auto layout file types"),
        description=_("Extra types only work with " "openoffice/libreoffice installed."),
        default=["pdf"],
        value_type=schema.Choice(vocabulary=SimpleVocabulary(FILE_TYPES_VOCAB)),
    )
    auto_convert = schema.Bool(
        title=_("Auto Convert"),
        description=_("Automatically convert files on creation " "and modification."),
        default=True,
    )
    override_contributor = schema.TextLine(
        title=_("Override Contributor"),
        description=_("What to override the contributor field on viewer with." "Leave blank to use document owner."),
        default=None,
        required=False,
    )
    override_organization = schema.TextLine(
        title=_("Override Contributor Organization"),
        description=_("What to override the organization field on viewer with." "Leave blank to use site title."),
        default=None,
        required=False,
    )
    override_base_resource_url = schema.URI(
        ...
        description=_(
            "If you're syncing your storage to another server you "
    ...
    width = schema.Int(
        title=_("Viewer Width"), description=_("Leave blank to take full width."), default=None, required=False
    )
    height = schema.Int(
        title=_("Viewer Height"),
        description=_("Default height to use for viewer (only for " "non-fullscreen mode)."),
        default=700,
    )
    show_sidebar = schema.Bool(
        title=_("Show sidebar"), description=_("Default to show sidebar on Document Viewer."), default=True
    )
    show_search = schema.Bool(title=_("Show search box"), description=_("On Document Viewer."), default=True)
    show_search_on_group_view = schema.Bool(
        title=_("Show search on group view"), description=_("Enable search on group view."), default=True
    )
    group_view_batch_size = schema.Int(
        title=_("Group View Batch Size"), description=_("For folders. Does not apply to topics."), default=20
    )
    async_quota_size = schema.Int(
        title=_("Async Quota Size"),
        description=_("Number of conversions to run at a time. " "The quota name assigned is `dv`."),
        default=3,
    )
...
class IDocumentViewerSettings(Interface):
    width = schema.Int(
        ...
        required=False,
        **_default(default_width, IGlobalDocumentViewerSettings["width"].default)
    )
    height = schema.Int(
        title=_("Viewer Height"),
        required=False,
        **_default(default_height, IGlobalDocumentViewerSettings["height"].default)
    )
    fullscreen = schema.Bool(
        title=_("Fullscreen Viewer"), description=_("Default to fullscreen viewer."), default=False
    )
    enable_indexation = schema.Bool(
        title=_("Make searchable"),
        description=_(
        ...
        **_default(default_enable_indexation, IGlobalDocumentViewerSettings["width"].default)
    )
    show_sidebar = schema.Bool(
        title=_("Show sidebar"),
        description=_("Default to show sidebar."),
        required=False,
        **_default(default_show_sidebar, IGlobalDocumentViewerSettings["width"].default)
    )
    show_search = schema.Bool(
        title=_("Show search box"), **_default(default_show_search, IGlobalDocumentViewerSettings["width"].default)
    )
