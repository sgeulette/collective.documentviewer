...
class StorageTest(BaseTest):
    def test_retrieve_correct_resource_location_old_storage(self):
        ...
        self.assertEquals(storage.getResourceDirectory(obj=fi), join(_dir, fi.UID()))
    ...
    def test_retrieve_correct_resource_location_new_storage(self):
        ...
        uid = fi.UID()
        self.assertEquals(storage.getResourceDirectory(obj=fi), join(_dir, uid[0], uid[1], uid))
    ...
    def test_get_correct_rel_url_for_old_storage(self):
        ...
        del settings._metadata["storage_version"]
        uid = fi.UID()
        self.assertEquals(storage.getResourceRelURL(obj=fi), "@@dvpdffiles/%s" % uid)

    def test_get_correct_rel_url_for_new_storage(self):
        fi = self.createFile("test.pdf")
        uid = fi.UID()
        self.assertEquals(storage.getResourceRelURL(obj=fi), "@@dvpdffiles/%s/%s/%s" % (uid[0], uid[1], uid))
    ...
    def test_get_correct_rel_url_for_new_storage_with_resource_url(self):
        ...
        uid = fi.UID()
        self.assertEquals(storage.getResourceRelURL(obj=fi), "%s/%s/%s" % (uid[0], uid[1], uid))
    ...
    def test_storage_obsfucates_dir(self):
        ...
        notify(ObjectInitializedEvent(fi))
        settings = Settings(fi)
        self.assertTrue(exists(join(_dir, uid[0], uid[1], uid, settings.obfuscate_secret)))
        self.assertTrue(listdir(join(_dir, uid[0], uid[1], uid, settings.obfuscate_secret)) > 3)
    ...
    def test_storage_removes_obfuscation(self):
        ...
        self.assertTrue(not exists(join(_dir, uid[0], uid[1], uid, settings.obfuscate_secret)))
