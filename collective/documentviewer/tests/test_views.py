...
class PDFResourceTraverseTest(BaseTest):
    def test_filesystem_storage_works(self):
        ...
        fiobj = self.portal.unrestrictedTraverse("@@dvpdffiles/%s/%s/%s/small/dump_1.gif" % (uid[0], uid[1], uid))
        self.assertEquals(fiobj.context.path, join(_dir, uid[0], uid[1], uid, "small", "dump_1.gif"))
        fiobj = self.portal.unrestrictedTraverse("@@dvpdffiles/%s/%s/%s/normal/dump_1.gif" % (uid[0], uid[1], uid))
        self.assertEquals(fiobj.context.path, join(_dir, uid[0], uid[1], uid, "normal", "dump_1.gif"))
        fiobj = self.portal.unrestrictedTraverse("@@dvpdffiles/%s/%s/%s/large/dump_1.gif" % (uid[0], uid[1], uid))
        self.assertEquals(fiobj.context.path, join(_dir, uid[0], uid[1], uid, "large", "dump_1.gif"))
        fiobj = self.portal.unrestrictedTraverse("@@dvpdffiles/%s/%s/%s/text/dump_1.txt" % (uid[0], uid[1], uid))
        self.assertEquals(fiobj.context.path, join(_dir, uid[0], uid[1], uid, "text", "dump_1.txt"))
    ...
    def test_filesystem_old_storage_works(self):
        ...
        fiobj = self.portal.unrestrictedTraverse("@@dvpdffiles/%s/small/dump_1.gif" % uid)
        self.assertEquals(fiobj.context.path, join(_dir, uid, "small", "dump_1.gif"))
    ...
    def test_filesystem_missing_gives_404(self):
        ...
        uid = fi.UID()
        self.assertRaises(
            KeyError, self.portal.unrestrictedTraverse, "@@dvpdffiles/%s/%s/%s/small/foobar.gif" % (uid[0], uid[1], uid)
        )
