...
def convert_all(context):
    ...
    for brain in catalog(object_provides=OBJECT_PROVIDES):
        ...
        if not allowedDocumentType(file_item, gsettings.auto_layout_file_types):
            continue
        ...
        if file.getLayout() == "documentviewer":
            ...
            if settings.storage_version == 1:
                if settings.storage_type == "File":
                    current_location = storage.getResourceDirectory(gsettings=gsettings, settings=settings)
                    if not exists(current_location):
                        raise Exception("oops, can't find storage location %s" % (current_location))

                    settings.storage_version = STORAGE_VERSION
                    new_location = storage.getResourceDirectory(gsettings=gsettings, settings=settings)
                    # only make base
                    mkdir_p(os.path.sep.join(new_location.split(os.path.sep)[:-1]))
